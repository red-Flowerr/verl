...
process_checkpoint() {
    ...
    if [ ! -d "$base_checkpoint_path/$step_tag/$actor_dir/huggingface" ]; then
        # Call the model conversion command here
        echo "huggingface directory not found, converting model..."
        # Your conversion command goes here
        python model_merger.py --backend fsdp --hf_model_path $init_model_path --local_dir $base_checkpoint_path/$step_tag/$actor_dir --target_dir $base_checkpoint_path/$step_tag/$actor_dir/huggingface
        # call copy tokenizer to copy
        copy_tokenizer_files "$base_checkpoint_path/$step_tag/$actor_dir/huggingface" "$init_model_path"
        echo "Model convert done for $base_checkpoint_path/$step_tag/$actor_dir" >&2
    fi
